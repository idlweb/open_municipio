...
def make_website_skeleton():
    ...
    with hide('commands'):
        with cd(env.web_root):
            ...
            sudo('touch %(app_domain)s/log/logfile' % env)
            sudo('chmod g+w %(app_domain)s/log/logfile' % env)           
            sudo('touch %(app_domain)s/log/webapp.log' % env)
            sudo('chmod g+w %(app_domain)s/log/webapp.log' % env)           
